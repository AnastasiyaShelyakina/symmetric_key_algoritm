...
std::string AES_encryptString(const std::string& input) {
	...
	AES aes(key);
	unsigned char* encrypted = aes.Cipher((unsigned char*)input.c_str());
	std::string encryptedString((char*)encrypted);
...
std::string AES_decryptString(const std::string& input) {
	unsigned char key[16] = { 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF };
	AES aes(key);
	unsigned char* decrypted = aes.InvCipher((unsigned char*)input.c_str());
	std::string decryptedString((char*)decrypted);
