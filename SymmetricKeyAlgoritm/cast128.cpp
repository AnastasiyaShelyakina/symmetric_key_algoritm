...
Cast128::Block Cast128::decrypt(const Key key, const Block msg)
{
	Block decryptedMsg = go(key, msg, DECRYPT);
	return decryptedMsg;
}
...
void Cast128::generateKey(Cast128::Key& key) {
	std::srand(std::time(0));
	for (int i = 0; i < KEY_SIZE; ++i) {
		key[i] = static_cast<uint32_t>(std::rand());
	}
}
